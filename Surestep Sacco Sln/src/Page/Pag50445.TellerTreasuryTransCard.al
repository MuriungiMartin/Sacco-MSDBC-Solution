...
Page 50445 "Teller & Treasury Trans Card"
{
    DeleteAllowed = false;
    PageType = Card;
        ...
        area(processing)
        {
            ...
            action(Receive)
            {
                ...
                begin
                    ...
                    ObjGLTransactions.SetRange("Document No.", No);
    ...
    begin
        ...
        if TellerTill.Find('-') then begin
            TellerTill.CalcFields(TellerTill.Balance);
            "Source Account Balance" := TellerTill.Balance;
            Modify;
        end;

        if TellerTill.Get("From Account") then begin
            TellerTill.CalcFields(TellerTill.Balance);
            "Actual Teller Till Balance" := TellerTill.Balance
        end;
        ...
        if TellerTill.Find('-') then begin
            TellerTill.CalcFields(TellerTill.Balance);
            "Destination Account Balance" := TellerTill.Balance;
