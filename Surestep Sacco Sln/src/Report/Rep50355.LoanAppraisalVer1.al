...
Report 50355 "Loan Appraisal Ver1"
{
    ...
    dataset
    {
        dataitem("Loans Register"; "Loans Register")
        {
            ...
            begin
                ...
                FnRunGetQualificatioPerIncome("Loan  No.");//=============================================================================Income Qualification
    ...
    var
        ObjCustRec: Record "Members Register";
        ObjGenSetUp: Record "Sacco General Set-Up";
        ObjCustRecord: Record "Members Register";
        VarTShares: Decimal;
        VarTLoans: Decimal;
        ...
        ObjCustDiv: Record "Members Register";
        ObjDivProg: Record "Dividends Progression";
        VarCDiv: Decimal;
        VarBDate: Date;
        ObjCustR: Record "Members Register";
        VarBasicPay: Decimal;
    ...
    local procedure FnRunGetQualificatioPerIncome(VarLoanNo: Code[30])
    begin
        ObjLoans.Reset;
        ObjLoans.SetRange(ObjLoans."Loan  No.", VarLoanNo);
        if ObjLoans.FindSet then begin
    ...
    begin
		...
		ObjLoanG.RESET;
		ObjLoanG.SETRANGE(ObjLoanG."Loan No",VarLoanNo);
		IF ObjLoanG.FINDFIRST THEN BEGIN
		  ObjLoanG.CALCFIELDS("Total Amount Guaranteed","Total Loan Risk");
		  VarGShares:=ObjLoanG."Total Amount Guaranteed"-ObjLoanG."Total Loan Risk";
		  END;
		...
		ObjLoans.RESET;
		ObjLoans.SETRANGE(ObjLoans."Loan  No.",VarLoanNo);
		ObjLoans.SETFILTER(ObjLoans."Loan Product Type",'<>%1|<>%2|<>%3','315','317','324');
		IF ObjLoans.FINDSET THEN
		  BEGIN
			VarDepMultiplier:=FnRunGetQualificationPerDeposits(ObjLoans."Loan  No.",ObjLoans."Client Code");
			VarDepX:=(VarDepMultiplier-((VarTotalLoansnotSecuredbyCollateral+VarTotalCollateralValue))-VarBridgeBal);
			END;
		ObjLoans.RESET;
		ObjLoans.SETRANGE(ObjLoans."Loan  No.",VarLoanNo);
		ObjLoans.SETFILTER(ObjLoans."Loan Product Type",'%1|%2|%3','315','317','324');
		IF ObjLoans.FINDSET THEN
		  BEGIN
			VarDepX:="Loans Register"."Requested Amount";
			END;
		//==============================================================================================Check Deposit Qualification FOSA
		IF ObjLoanType.GET(VarLoanType) THEN
		  BEGIN
		   IF ObjLoanType."FOSA Loan Shares Min Amount"<>0 THEN
			 BEGIN
			   IF VarCshares>ObjLoanType."FOSA Loan Shares Min Amount" THEN
				 BEGIN
				   VarDepX:="Loans Register"."Requested Amount"
				   END ELSE
				   VarDepX:=0
		...
		BEGIN
		  {IF (ObjLoans."Member House Group"='')  THEN
		  BEGIN
			IF ObjLoans."Member Deposits">=ObjLoans."Requested Amount" THEN
			BEGIN
			  VarDepX:=ObjLoans."Member Deposits"*(ObjGenSetUp."Self Guarantee Multiplier"/100);
			  VarTotalGuaranteed:=ObjLoans."Member Deposits"*(ObjGenSetUp."Self Guarantee Multiplier"/100);
			END ELSE
			  ...
			  IF ObjLoanGuarantors.FINDSET THEN BEGIN
				...
				VarTotalGuaranteed:=ObjLoanGuarantors."Amont Guaranteed";
			  END;
		   END;}
			ObjLoanGuarantors.RESET;
			ObjLoanGuarantors.SETRANGE(ObjLoanGuarantors."Loan No",VarLoanNo);
			IF ObjLoanGuarantors.FINDSET THEN BEGIN
			  ...
			  VarTotalGuaranteed:=ObjLoanGuarantors."Amont Guaranteed";
			END;
		   IF (ObjLoans."Member House Group"<>'') AND (ObjLoans.Source=ObjLoans.Source::BOSA) THEN
			  VarTotalGuaranteed:=SFactory.FnGetGroupNetworth(ObjLoans."Member House Group");
		END;
		ObjLoanGuarantors.RESET;
		ObjLoanGuarantors.SETRANGE(ObjLoanGuarantors."Loan No",VarLoanNo);
		IF ObjLoanGuarantors.FINDSET=FALSE THEN BEGIN
		  VarTotalGuaranteed:=0;
		  END;
		//=============================================================================Check Existance of Collateral
		VarCollateralSecurityOffered:=FALSE;
		ObjSecurities.RESET;
		ObjSecurities.SETRANGE(ObjSecurities."Loan No",VarLoanNo);
		ObjSecurities.SETFILTER(ObjSecurities."Collateral Registe Doc",'<>%1','');
		IF ObjSecurities.FINDSET=TRUE THEN BEGIN
		  VarTotalGuaranteed:="Loans Register"."Requested Amount";
		  VarCollateralSecurityOffered:=TRUE;
		  END;
		//=============================================================================End Check Existance of Collateral
		VarSelfGuarantee:=FALSE;
		ObjLoans.RESET;
		ObjLoans.SETRANGE(ObjLoans."Loan  No.",VarLoanNo);
		IF ObjLoans.FINDSET THEN
		  BEGIN
		  ObjLoanGuarantors.RESET;
		  ObjLoanGuarantors.SETRANGE(ObjLoanGuarantors."Loan No",VarLoanNo);
		  //ObjLoanGuarantors.SETFILTER(ObjLoanGuarantors."Member No",ObjLoans."Client Code");
		  //ObjLoanGuarantors.SETRANGE(ObjLoanGuarantors."Self Guarantee",TRUE);
		  IF ObjLoanGuarantors.FINDSET THEN
			BEGIN
			IF (ObjLoanGuarantors.COUNT=1) AND (ObjLoanGuarantors."Member No"=ObjLoans."Client Code") THEN
			  VarTotalGuaranteed:=ObjLoanGuarantors."Amont Guaranteed";
			  VarSelfGuarantee:=TRUE;
			  END;
		  END;
		IF (VarCollateralSecurityOffered=TRUE) AND (VarSelfGuarantee=TRUE) THEN
		  VarTotalGuaranteed:="Loans Register"."Requested Amount";*/
